"""Tests for the shipped ready-to-use workflows and tasks of the Optical module.

The shipped workflow/task set is **not** hand-maintained here: it is the set computed by
``discover_shipped_workflows()`` and ``discover_shipped_tasks()`` (the same discovery the
migration generator uses), so these tests cannot drift from the workflows that are
actually shipped. The migration ``WorkflowMigration`` record carries the workflow ``name``
and ``target`` but not its module, so the module for each discovered workflow is resolved
by walking the ``orchestrator.optical.workflows`` package once and indexing the decorated
workflow functions by their shipped name.

For every discovered workflow or task these tests verify that it (a) is importable from
its module, (b) is a ``Workflow`` instance whose ``.name`` matches, (c) has the target
...
def _modules_by_workflow_name() -> dict[str, str]:
    """Index every decorated shipped workflow or task function by its shipped name.
...
_DISCOVERED_TASKS = discover_shipped_tasks()
...
#: ``(name, module)`` pairs, one per discovered shipped task (all target SYSTEM).
TASK_PARAMS = sorted((task.name, _WORKFLOW_MODULES[task.name]) for task in _DISCOVERED_TASKS)
...
@pytest.mark.parametrize(("name", "module_name"), TASK_PARAMS)
def test_shipped_task_exists_with_matching_name_and_system_target(name, module_name) -> None:
    """Every discovered task is importable, is a ``Workflow`` and targets SYSTEM."""
    task = getattr(import_module(module_name), name)

    assert isinstance(task, Workflow)
    assert task.name == name
    assert task.target == Target.SYSTEM


@pytest.mark.parametrize(("name", "module_name"), TASK_PARAMS)
def test_shipped_tasks_instantiate_through_lazy_workflow_instance(name, module_name) -> None:
    """Every discovered task instantiates through ``LazyWorkflowInstance``."""
    lazy = LazyWorkflowInstance(module_name, name)
    task = lazy.instantiate()
