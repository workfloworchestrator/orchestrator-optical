...
from products.services.optical_device import get_flex_client
from products.services.optical_device_port import flexils_check_port_is_in_manualmode2_else_set_it
from services.nokia import FlexilsClient, TL1CommandDeniedError
...
def _find_or_create_oel(
    ...
    flex = get_flex_client(source_device)
    aid = oel_name[:127]
    flex.ent_oel(
        aid=aid,
        label=oel_label,
...
def _get_flexils_name_client_tributary(device: OpticalDeviceBlock, port_name: str) -> tuple[str, FlexilsClient, str]:
...
def _(
    ...
    flex = get_flex_client(src_device)
...
def _(
    ...
    for port in optical_spectrum_section.add_drop_ports:
        ...
        if od.fqdn.startswith(flex.tid):
            continue
        remote_flex = get_flex_client(od)
