...
def _(
    ...
    for port_name, central_frequency, mode, description in zip(
        ...
        uri, _, _, _, port_id, _ = g30_port_navigator_node_from_port_name(optical_device, port_name)
...
def _(
    ...
    navigator, _, _, _, port_id, _ = g30_port_navigator_node_from_port_name(optical_device, port_name)
...
def delete_transponder_crossconnect(
    ...
    Returns:
        dict: {
            "message": f"{optical_device.fqdn} {client_port_name}: {variable message}",
...
def _(
    ...
    if card.required_type == "CHM1":
        result["message"] = (
            f"{optical_device.fqdn} {client_port_name}: no need to delete any xcon,"
    ...
    if not eth_key:
        result["message"] = (
            f"{optical_device.fqdn} {client_port_name}:"
...
def factory_reset_transponder_lines(
    ...
    """
    return attribute_dispatch_base(factory_reset_transponder_lines, "platform", optical_device.platform)


@factory_reset_transponder_lines.register(Platform.Groove_G30)
def _(
    optical_device: OpticalDeviceBlock,
    line_port_names: list[str],
) -> dict[str, Any]:
...
@factory_reset_transponder_lines.register(Platform.GX_G42)
def _(
    optical_device: OpticalDeviceBlock,
    line_port_names: list[str],
) -> dict[str, Any]:
