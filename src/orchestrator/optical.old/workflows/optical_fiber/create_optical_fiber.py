...
def subscription_description(subscription: OpticalFiberProvisioning) -> str:
...
def initial_input_form_generator(product_name: str) -> FormGenerator:
    src_optical_device_choice: type[Choice] = active_subscription_selector(
        "OpticalDevice", prompt="This fiber connects this node:"
    )  # pyright: ignore[reportInvalidTypeForm]
    dst_optical_device_choice: type[Choice] = active_subscription_selector(
        "OpticalDevice", prompt="to this other node:"
    )  # pyright: ignore[reportInvalidTypeForm]

    class SelectOpticalDevicesForm(FormPage):
        model_config = ConfigDict(title=product_name)

        sub_id_device_a: src_optical_device_choice
        sub_id_device_b: dst_optical_device_choice
    ...
    src_optical_port_selector: type[Choice] = unused_optical_port_selector(user_input_dict["sub_id_device_a"])  # pyright: ignore[reportInvalidTypeForm]
    dst_optical_port_selector: type[Choice] = unused_optical_port_selector(user_input_dict["sub_id_device_b"])  # pyright: ignore[reportInvalidTypeForm]

    class SelectOpticalPortsForm(FormPage):
        model_config = ConfigDict(title=product_name)

        name_port_a: src_optical_port_selector
        name_port_b: dst_optical_port_selector
        ...
        def validate_data(self) -> "SelectOpticalPortsForm":
            for sub_id, port_name in (
                ...
                device_port_sivs = subscription_instance_values_by_block_type_depending_on_instance_id(
                    ...
                    depending_on_instance_id=str(sid),
                    states=[
...
def retrieve_used_passbands(subscription: OpticalFiberProvisioning) -> State:
    for port in subscription.optical_fiber.terminations:
        ...
        if device.device_type in [DeviceType.ROADM, DeviceType.TransponderAndOADM]:
            ports_spectral_occupation: dict[str, list[tuple[int, int]]] = retrieve_ports_spectral_occupations(device)
