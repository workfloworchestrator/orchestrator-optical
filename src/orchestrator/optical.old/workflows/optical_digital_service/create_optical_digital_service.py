...
Queries each ROADM/OADM along the path to retrieve current spectral occupancy and updates the `used_passbands`
field on each port. This keeps database in sync with actual device state for future path calculations.
...
When reusing existing channels, updates the OSNC labels to include the new service's fXcY identifier.
Multiple services sharing channels have labels like: "f001c01+f002c03".
...
- **Nokia Groove G30**: Transponder/transceiver
- **Nokia GX G42**: Transponder/transceiver
- **Nokia FlexILS**: ROADM with wavelength-selective switching
...
def initial_input_form_generator(product_name: str) -> FormGenerator:  # noqa: ARG001
    PartnerChoice = active_subscription_selector("Partner", prompt="Who is the user of this service?")  # noqa: N806
    ...
    NodeAChoice = optical_device_selector_of_types(  # noqa: N806
        device_types=transceivers_types,
        prompt="This service connects this node: ",
    )

    NodeBChoice = optical_device_selector_of_types(  # noqa: N806
        device_types=transceivers_types,
        prompt="...to this other node: ",
    )

    ServiceTypeChoice = Choice("What service do you want?", [(speed, speed.value) for speed in ClientSpeednType])  # noqa: N806
    ...
    if are_channel_already_provisioned:
        ...
        ExcludeOpticalDeviceChoiceList = multiple_optical_device_selector(  # noqa: N806
            device_types=line_system_types,
            prompt="Do *not* pass through these Optical Devices",
        )

        ExcludeSpanChoiceList = multiple_optical_fiber_selector(  # noqa: N806
...
def construct_optical_digital_service_model(  # noqa: PLR0915
    product: UUIDstr,
    user_id: UUIDstr,
    id_node_a: UUIDstr,
    id_node_b: UUIDstr,
...
def divide_path_into_sections(
    ...
    if len(channels) == 2:  # noqa: PLR2004
        ...
        # Build a new list of UUID strings — extract subscription_instance_id from the domain blocks
        channel_2_path = list(optical_path)
        channel_2_path[0] = first_add_drop_port.subscription_instance_id
        channel_2_path[-1] = last_add_drop_port.subscription_instance_id

        store_list_of_ports_into_spectrum_sections(channel_2_path, channels[1].optical_spectrum)
