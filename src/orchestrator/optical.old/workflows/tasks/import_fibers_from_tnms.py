...
from services.nokia import tnms_client
...
def fecth_fibers_from_tnms(
    ...
    for f in fibers:
        ...
        if len(access_ports) != 2 or not isinstance(access_ports, list):  # noqa: PLR2004
            continue
...
def find_devices_subscriptions(fetched_fibers: list[dict[str, str]]) -> State:
    """Find subscriptions for the devices in the TNMS data using their FQDN."""
    device_sub_id_by_fqdn = {}
    for fiber in fetched_fibers:
        for name in [fiber["src_device_name"], fiber["dst_device_name"]]:
            fqdn = f"{name}.garr.net"
            if fqdn not in device_sub_id_by_fqdn:
                ...
                if len(device_subscriptions) != 1:
                    msg = f"Device with FQDN {fqdn} not found or not unique"
                    raise ValueError(msg)
                device_sub_id_by_fqdn[fqdn] = device_subscriptions[0].subscription_id

    return {"device_sub_id_by_fqdn": device_sub_id_by_fqdn}
...
def create_workflow_inputs(sifted_fibers: list[dict[str, str]], device_sub_id_by_fqdn: dict[str, UUIDstr]) -> State:
    ...
    for fiber in sifted_fibers:
        src_fqdn = f"{fiber['src_device_name']}.garr.net"
        dst_fqdn = f"{fiber['dst_device_name']}.garr.net"
        ...
        user_inputs.append(
            {
                ...
                "fiber_types": None,
                "lengths": None,
                "sub_id_device_a": device_sub_id_by_fqdn[src_fqdn],
                "sub_id_device_b": device_sub_id_by_fqdn[dst_fqdn],
            }
        )
        user_inputs.append(
            {
