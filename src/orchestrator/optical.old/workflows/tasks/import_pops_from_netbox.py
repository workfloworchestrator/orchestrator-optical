...
def fetch_input_data() -> State:
    ...
    input_data = []
    for pop in pops:
        if pop.custom_fields.get("garrx_db_id") is None or not match(r"^[A-Z0-9]{4}$", pop.slug.upper()):
            continue
        input_data.append(
            {
                "netbox_id": pop.id,
...
def start_sub_workflows(workflow_input_forms: list[State]) -> State:
    ...
    while workflow_input_forms:
        ...
        with db.database_scope():
            process_id = start_process("create_pop", user_inputs=user_inputs, user="SYSTEM")
