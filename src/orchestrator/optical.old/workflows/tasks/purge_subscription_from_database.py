...
def initial_input_form_generator() -> FormGenerator:
    class InputForm(FormPage):
        achtung: Achtung
        subscription_id: UUIDstr = Field(..., title="Subscription ID", description="UUID of the hung subscription")
...
def check_dependencies(subscription_id: str) -> State:
    ...
    if sub.insync is True:
        raise ValueError(f"Subscription {subscription_id} is in sync, use terminate workflow instead.")
    ...
    if dependent_instance_relations:
        for relation in dependent_instance_relations:
            ...
            owner_sub_id = (
                session.query(SubscriptionInstanceTable).filter_by(subscription_instance_id=dependent_instance_id).one()
            ).subscription_id
            if str(owner_sub_id) != subscription_id and str(owner_sub_id) not in dependent_subscriptions_ids:
                dependent_subscriptions_ids.add(str(owner_sub_id))

    if dependent_subscriptions_ids:
        raise ValueError("Cannot delete: found dependent subscriptions: " + ", ".join(dependent_subscriptions_ids))
...
def delete_db_entries(subscription_id: str) -> State:
    ...
    processes_subscriptions = (
        session.query(ProcessSubscriptionTable).filter_by(subscription_id=sub.subscription_id).all()
    )
    ...
    if pids:
        ...
        num_del = session.query(InputStateTable).filter(InputStateTable.process_id.in_(pids)).delete()
        ...
        num_del = session.query(ProcessTable).filter(ProcessTable.process_id.in_(pids)).delete()
