...
def initial_input_form_generator() -> FormGenerator:
    class CSVDataForm(FormPage):
        ...
        def validate_csv(self) -> "CSVDataForm":
            ...
            required_headers = set(h.strip() for h in CsvData.__metadata__[0].default.split(","))
            ...
            for idx, device in enumerate(devices):
                ...
                if not match_pop_code:
                    raise ValueError(f"Invalid PoP code '{device['pop_code']} at row {idx + 1}")
...
def find_pops_and_duplicate_addresses(
    ...
    for device in csv_data:
        for address in ["lo_ip", "mngmt_ip"]:
            ...
            if duplicates:
                raise ValueError(f"IP {device[address]} is already in use by another device")

        if device["pop_code"] not in pops_ids:
            subscriptions_matching_pop_code = subscriptions_by_product_type_and_instance_value(
                "PoP",
                "code",
                device["pop_code"],
                status=[SubscriptionLifecycle.ACTIVE],
            ...
            if len(subscriptions_matching_pop_code) != 1:
                raise ValueError(f"PoP code {device['pop_code']} not found or not unique")
            pops_ids[device["pop_code"]] = subscriptions_matching_pop_code[0].subscription_id
...
def start_sub_workflows(workflow_input_forms: list[State]) -> State:
    ...
    while workflow_input_forms:
        ...
        with db.database_scope():
            process_id = start_process("create_optical_device", user_inputs=user_inputs, user="SYSTEM")
...
def bulk_create_optical_devices() -> StepList:
    return begin >> find_pops_and_duplicate_addresses >> create_workflow_inputs >> start_sub_workflows >> done
