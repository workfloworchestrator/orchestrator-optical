...
def initial_input_form_generator() -> FormGenerator:
    class CSVDataForm(FormPage):
        ...
        def _validate_and_convert_device_ids(self, fiber: dict, row: int) -> dict:
            for device_key in ["optical_device_a", "optical_device_b"]:
                ...
                if not subscriptions:
                    raise ValueError(f"FQDN {fiber[device_key]} not found or not active at row {row}")
                if len(subscriptions) > 1:
                    raise ValueError(f"FQDN {fiber[device_key]} is not unique at row {row}")
        ...
        def _validate_and_convert_fiber_specifications(self, fiber: dict, row: int) -> dict:
            try:
                fiber["type1-type2-..."] = (
                    list(map(FiberType, fiber["type1-type2-..."].split("-"))) if fiber["type1-type2-..."] else None
                )
            except ValueError:
                raise ValueError(
            ...
            try:
                fiber["length1-length2-..."] = (
                    list(map(int, fiber["length1-length2-..."].split("-"))) if fiber["length1-length2-..."] else None
                )
            except ValueError:
                raise ValueError(
...
def bulk_create_optical_fibers() -> StepList:
    return begin >> create_workflow_inputs >> start_sub_workflows >> done
