...
class NamedValueSetItem(YangBaseModel):
    ...
    name: Annotated[str, AfterValidator(lambda v: check_pattern("^(?:([A-Za-z0-9_\\-.,]*))$", v))] = Field(
        json_schema_extra={"is_config": True}, description="Name of the value item.", min_length=1, max_length=64
    )
    value: str | None = Field(
        json_schema_extra={"is_config": True}, description="Value item.", min_length=0, max_length=1024, default=None
    )
...
class DbEntryItem(YangBaseModel):
    ...
    name: Annotated[str, AfterValidator(lambda v: check_pattern("^(?:([A-Za-z0-9_\\-.,]*))$", v))] = Field(
        json_schema_extra={"is_config": True}, description="Name of the DB entry.", min_length=1, max_length=64
    )
    value: str | None = Field(
        json_schema_extra={"is_config": True},
        description="Value associated with the entry.",
        min_length=0,
        max_length=1024,
        default=None,
    )
    named_value_set: RestconfList[NamedValueSetItem] | None = Field(
        json_schema_extra={"is_config": True},
        description="Set of named values associated with the DB entry. Useful for storing multi-attribute information (record).",
        default=None,
        alias="named-value-set",
    )
...
class UserData(YangBaseModel):
    ...
    db_entry: RestconfList[DbEntryItem] | None = Field(
        json_schema_extra={"is_config": True},
        description="DB for storing multiple user created entries keyed by name.",
        default=None,
        alias="db-entry",
    )
...
class IoaUserDataData(YangBaseModel):
    ...
    user_data: UserData | None = Field(
        json_schema_extra={"is_config": True},
        description="Container to store user created data on the network element.",
        default=None,
        alias="ioa-user-data:user-data",
    )
