...
def update_optical_node_fields(
    ...
    """Update the common Optical Node fields and keep the customer in sync with the location."""
    node = subscription.optical_node  # type: ignore[unresolved-attribute]
    node.location = location_block_from_subscription(location_id)
    node.pqdn = pqdn
    node.optical_management_ip = optical_management_ip
    node.optical_loopback_ip = optical_loopback_ip
