...
the subscription)::
    ...
    user_input_dict = yield from modify_fiber_patch_form_pages(block)
...
from orchestrator.optical.products.product_blocks.optical_pipe.fiber_patch import OpticalFiberPatchBlock
...
def modify_fiber_patch_form_pages(pipe: OpticalFiberPatchBlock) -> FormGenerator:
...
def modify_fiber_patch_form_generator(
    ...
    subscription_model: type[SubscriptionModel] = OpticalFiberPatchSubscription,
    ...
    page, the shipped page sequence (:func:`modify_fiber_patch_form_pages`) and
...
MODIFY_FIBER_PATCH_BLOCK_STEPS: StepList = begin >> update_optical_pipe_block >> save_optical_module_block
