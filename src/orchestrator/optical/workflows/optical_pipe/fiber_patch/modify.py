...
This module ships the ready-to-use ``modify_fiber_patch`` workflow for the
...
sequence in one line and adding their own pages::
    ...
    user_input_dict = yield from modify_fiber_patch_form_pages(
...
def modify_fiber_patch_form(
    subscription: SubscriptionModel,
    block_field_name: str = "optical_pipe",
) -> type[FormPage]:
    """Return the modify FormPage of the Optical Fiber Patch subscription.
...
def modify_fiber_patch_form_pages(
    subscription: SubscriptionModel,
    block_field_name: str = "optical_pipe",
) -> FormGenerator:
    """Yield the FormPage of the Optical Fiber Patch modify form.
    ...
    """
    user_input = yield modify_fiber_patch_form(subscription, block_field_name)
    return user_input.model_dump()


def modify_fiber_patch_form_generator(
    subscription_id: UUIDstr,
    subscription_model: type[SubscriptionModel] = OpticalFiberPatch,
    block_field_name: str = "optical_pipe",
) -> FormGenerator:
    """Generate the initial input form for modifying an Optical Fiber Patch subscription.
    ...
    page sequence (:func:`modify_fiber_patch_form_pages`) and the summary
    form.
    ...
    Args:
        ...
        subscription_model: The ACTIVE subscription model class of the Optical
            Fiber Patch product. Consumers that compose the shipped block
...
MODIFY_FIBER_PATCH_BLOCK_STEPS: StepList = begin >> update_optical_pipe_block >> save_optical_pipe_block


@modify_workflow(initial_input_form=modify_fiber_patch_form_generator)
def modify_fiber_patch() -> StepList:
    """Workflow to modify an existing Optical Fiber Patch subscription.

    The workflow is valid for the shipped :class:`OpticalFiberPatch` product
