"""Reconcile Optical Fiber Patch workflow.

This module ships the ready-to-use ``reconcile_fiber_patch`` workflow for the
...
their own ``@reconcile_workflow`` with :data:`RECONCILE_FIBER_PATCH_BLOCK_STEPS`.
"""
...
#: Reconcile steps of the Optical Fiber Patch family. Re-applies the
...
RECONCILE_FIBER_PATCH_BLOCK_STEPS: StepList = (
    begin
    >> configure_pipe_terminations
    >> retrieve_optical_pipe_used_passbands
    >> save_optical_pipe_block
...
@reconcile_workflow()
def reconcile_fiber_patch() -> StepList:
    """Workflow to reconcile an Optical Fiber Patch subscription.
    ...
    shipped ``OpticalFiberPatch`` product type only; consumers with their own
    product type compose their own reconcile workflow with the shipped parts.
    """
    return (
        begin
        >> load_optical_pipe_block
        >> RECONCILE_FIBER_PATCH_BLOCK_STEPS
        >> set_optical_pipe_subscription_description
    )
