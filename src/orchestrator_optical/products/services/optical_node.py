...
from orchestrator_optical.workflows.shared import subscription_instances_by_block_type
...
class FlexilsGneProvider:
    ...
    def _initialize_cache(cls):
        ...
        sub_instances = subscription_instances_by_block_type(
            product_block_type="NokiaFlexILSNode",
        )
        ...
        for si in sub_instances:
            ...
            if dev.mngmt_ip and dev.pop.latitude and dev.pop.longitude:
                ...
                vector = (x, y, z)
                cls._tid_ip_xyz_of_gnes.append((dev.pqdn, dev.mngmt_ip, vector))
    ...
    def find_closest_gne_ip(cls, ne: NokiaFlexILSNodeBlock) -> IPAddress:  # noqa: D102
        ...
        for gne_tid, gne_ip in nearest_gne_ips:
            try:
                ...
                if any(target_aid in node["NENAME"] or target_gmpls_id in node["ROUTERID"] for node in parsed_data):
        ...
        msg = f"No GNE found for NE {ne.pqdn} within these nodes: {nearest_gne_ips}"
...
def retrieve_omses_terminating_on_device(
    ...
    """
    return attribute_dispatch_base(retrieve_omses_terminating_on_device, "vendor_and_platform", optical_node.vendor_and_platform)
...
def retrieve_ports_spectral_occupations(
    ...
    """
    return attribute_dispatch_base(retrieve_ports_spectral_occupations, "vendor_and_platform", optical_node.vendor_and_platform)
