...
if TYPE_CHECKING:
    from orchestrator_optical.services.nokia.g30.data_navigators.ne import PortItemNode, SubportItemNode
...
def g30_port_navigator_node_from_port_name(
    ...
    """
    g30 = get_optical_node_client(g30_device_block)
...
def get_optical_node_ports_names(optical_node: OpticalNodeBlockUnion) -> list[str]:
    ...
    Args:
        optical_node: OpticalDevice of which the optical ports are to be retrieved
...
def set_port_description(optical_node: OpticalNodeBlockUnion, port_name: str, port_description: str) -> dict[str, Any]:  # noqa: ARG001
    ...
    Args:
        optical_node: OpticalDevice of which the optical port is to be modified
    ...
    """
    return attribute_dispatch_base(set_port_description, "vendor_and_platform", optical_node.vendor_and_platform)
...
def set_channel_description(optical_node: OpticalNodeBlockUnion, facility_id: str, description: str) -> dict[str, Any]:  # noqa: ARG001
    ...
    Raises:
        ...
        ValueError: in case the configuration failed
    """
    return attribute_dispatch_base(set_channel_description, "vendor_and_platform", optical_node.vendor_and_platform)
...
def configure_termination_when_attaching_new_fiber(
    ...
    Args:
        optical_node: OpticalDevice of which the optical port is to be configured
    ...
    return attribute_dispatch_base(
        ...
        "vendor_and_platform",
        optical_node.vendor_and_platform,
    )
...
def flexils_check_port_is_in_manualmode2_else_set_it(
    ...
    if scg["INTFTYP"] != "MANUALMODE-2":
        ...
        if card["TYPE"] in ["FSM", "FRM"]:
            ...
            set_port_admin_state(optical_node, port_name, "down")
        else:
            set_port_admin_state(optical_node, port_name, "maintenance")
...
def _(
    ...
    flexils_check_port_is_in_manualmode2_else_set_it(optical_node, port_name)
...
def _(  # noqa: PLR0915
    ...
    msg = (
        ...
        f"{remote_port.optical_node.vendor_and_platform}"
    )
    raise ValueError(msg)
...
def _(
    ...
    uri.update(
        ...
        connected_to=f"{remote_port.optical_node.pqdn} {remote_port.port_name}",
        admin_state="unlock",
    )
    return uri.retrieve(content="config", depth=2).model_dump()
...
def check_fiber_terminating_port(
    ...
    Args:
        optical_node: OpticalDevice of which the optical port is to be checked
    ...
    if remote_port.optical_node.vendor_and_platform == OpticalNodePlatform.FlexILS:
        ...
        if not checks:
            raise ValueError(
                json.dumps(
                    {
                        "optical_node": optical_node.pqdn,
                        "port_name": port_name,
                        "expected": {
                            "label": description,
                            "operstate": "IS",
...
def _(
    ...
    if (
        ...
        con_to_string = f"{remote_port.optical_node.pqdn} {remote_port.port_name}"
...
def _(
    ...
    checks = (
        ...
        and conf.connected_to == f"{remote_port.optical_node.pqdn} {remote_port.port_name}"
    )
    if not checks:
        raise ValueError(
            json.dumps(
                {
                    ...
                    "expected": {
                        "admin-status": "unlock",
                        "external-connectivity": "yes",
                        "connected-to": f"{remote_port.optical_node.pqdn} {remote_port.port_name}",
                    },
                    "actual": {
